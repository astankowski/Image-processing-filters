	...
	if (strcmp(axis, "y") == 0) {
		for (int y = 0; y < height; y++) {
			...
			for (int x = 0; x < width / 2; x++) {
				JSAMPROW pixel1 = &(row[x * num_components]);
				JSAMPROW pixel2 = &(row[(width - 1 - x) * num_components]);
				swap(pixel1, pixel2);
			}
		}
		...
		for (int y = 0; y < height / 2; y++) {
			...
			for (int x = 0; x < width; x++) {
				JSAMPROW pixel1 = &(row1[x * num_components]);
				JSAMPROW pixel2 = &(row2[x * num_components]);
				swap(pixel1, pixel2);
			}
		}		
	}
	...
	for (int y = 0; y < height; y++) {
		...
		for (int x = 0; x < width; x++) {
			...
			for (int i = 0; i < num_components; i++) {
				int new_value = (int)(times*(pixel[i] - 127) + 127);
				pixel[i] = clamp(0, 255, new_value);
			}
		}
	}
}   
...
void brightness(){
  for (int y = 0; y < height; y++) {
    ...
    for (int x = 0; x < width; x++) {
		...
		for (int i = 0; i < 3; i++) {
			int new_value = (int)(pixel[i] + (percent/100) * pixel[i]);
			pixel[i] =  clamp(0, 255, new_value);
		}
    }
  }
...
int main(int argc, char **argv){   
  ...
  struct arg_dbl *percent_arg = arg_dbl0("p", "percent" , "<percent>", "Change percent");
  ...
  struct arg_end *end = arg_end(10); // max error number = 10
  
  int nerrors;
  
  void *argtable[] = {input_file_arg, output_file_arg, filter_arg, times_arg, help, axis_arg, direction_arg, percent_arg, end};
